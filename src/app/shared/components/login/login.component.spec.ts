...
describe('LoginComponent', () => {
  ...
  it(' should check length of username',() => {
    component.loginForm.get('username').setValue('XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX');
    expect(component.loginForm.get('username').hasError('maxlength')).toBeTruthy();
  ...
  it( ' Check when sign-in is clicked.', () => {
    ...
    const username = debugElement.query(By.css('[username=\'username\']'));
    const password = debugElement.query(By.css('[password=\'password\']'));
    ...
    component.loginForm.get('username').setValue('bhawnabhardwaj');
    component.loginForm.get('password').setValue('xyz@123');

    fixture.detectChanges();

    signinButton.triggerEventHandler('ngSubmit', null);
    formContact.triggerEventHandler('signin', { });
